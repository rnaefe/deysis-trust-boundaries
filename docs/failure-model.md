...
| Mock provider unavailable | `ProviderError` | Return failure; never convert to success | Host policy | Persisted row remains available to host policy |
...
| Worker restart | Process exit | The sample has no claim loop; a host orchestrator must recover rows | Queue policy | PostgreSQL rows are durable |
